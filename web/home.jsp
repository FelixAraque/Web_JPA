                    ...
                    <% 
                        if (provincias!=null){
                            for (int i=0;i<provincias.size();i++) {%>
                                <option value="<%=i %>"><%=provincias.get(i).getNombre()%></option>
                            <%}
                        }
                    %>  
                  </select>
                    ...
                    <option disabled selected value="null">Elija Municipio</option>
                        ...
                        if (municipios!=null){
                            for (int i=0;i<municipios.size();i++) { %>
                                <option value="<%=i %>"><%=municipios.get(i).getNombre()%></option>
                            <%} 
                        }%> 
                  </select>
                </div>
    ...
    <div class="row justify-content-center">
      ...
      <div class="col-md-6 my-2 d-flex">
        <div class="card text-left d-fill">
          <img src="img/<%=playas.get(i).getId()%>_<%=playas.get(i).getImagesList().get(0).getId()%>.jpg" class="img-fluid" alt="...">
          <div class="card-body">
            <h4 class="card-title"><%=playas.get(i).getNombre() %></h4>
            <p class="card-text"><%=playas.get(i).getDescripcion() %></p>
          </div>
          <%if (usuario!=null) {%>
          <a data-toggle="modal" data-target="#modalinfo" data-nombreplaya="<%=playas.get(i).getNombre()%>" data-idplaya="<%=playas.get(i).getId()%>"><i id="info" class="fa fa-info-circle text-danger fa-3x" aria-hidden="true"></i></a>
          <a href="Controller?op=detail&playa=<%= i%>"><i id="star" class="fa fa-star-half-o text-warning fa-3x" aria-hidden="true"></i></a>
          <%}%>
        </div>
      </div>
      <%}%>
...
<div id="tostadaMsg" style="position: absolute;top: 20px;left:40%;" data-delay="4000" class="toast bg-primary" role="alert" aria-live="assertive" aria-atomic="true">
